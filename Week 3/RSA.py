...
len = int(input("Nhập số chữ số của p và q:"))
...
while True:
    q = random.randint(10**(len - 1), 10**(len) - 1)
...
while True:
    e = random.randint(10**(2 * len - 2), 10**(2 * len - 1) - 1)
    if sympy.isprime(e):
        break
...
# Nội dung bản tin
name = "chaomunghaimuoinamthanhlaptruongdhcn"
